...
int Search(Hashtable ht, int maso, int &nprob)
{
    ...
    nprob = 0;
    int index = maso % ht.M;
    // Nếu tại vị trí Index không trống
    while (ht.table[index].Maso != 0 && nprob < ht.M)
    {
        if (ht.table[index].Maso == maso)
            return index;
        // Tiếp tục thăm dò nếu chưa tìm thấy
        nprob++;
        index = ((maso % ht.M) + nprob * nprob) % ht.M;
    }
    // Nếu số lần thăm dò vượt quá kích thước của bảng băm hoặc tại vị trí đó không có học sinh nào
